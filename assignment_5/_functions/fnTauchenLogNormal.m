function [vGrid, mTransition] = fnTauchenLogNormal(Parameters)
    ...
    SigmaEps        = Parameters.pSigmaEps;
    Rho             = Parameters.pRho;

    % Grid preparation
    vXGrid = zeros(M,1);
    vXGrid(1)       = -N * SigmaEps / sqrt(1 - Rho^2);
    vXGrid(end)     = N * SigmaEps / sqrt(1 - Rho^2);
    ...
    for jjj = 1:1:M
        for iii = 1:1:M
            IntPlus                 = (vXGrid(jjj) + d/2 - Rho *vXGrid(iii)) / SigmaEps;
            IntMinus                = (vXGrid(jjj) - d/2 - Rho *vXGrid(iii)) / SigmaEps;
