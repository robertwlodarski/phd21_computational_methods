function [AssetsPerCapita]          = fnInterestAssets(Interest,Grids,Parameters)
...
iK                      = ((Interest + pDelta) / (pAlpha * pA))^(1 / (pAlpha - 1))* vLabSupply;
    ...
    for zzz = 1:1:size(vGridZ,1)
        ...
        for aaa = 1:1:size(vGridA1,1)
            ...
            if (floor((iNumIterVFI-1) / iAccelerationInterv) == ((iNumIterVFI-1) / iAccelerationInterv) || iNumIterVFI <= iAccelerationStart)
                % Current values
                iWealth                     = vGridA1(aaa);
                iBudget                     = iWage * iLabour + (1+Interest) * iWealth;
...
AssetsPerCapita     = vGridA2' * iMarginalDist;
