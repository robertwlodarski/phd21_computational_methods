...
iGini                   = zeros(size(iK));
iMarginalDist           = zeros(size(iK,1),size(vGridA2,1));
...
while iErrorK > iTolK
    ...
    for ttt = 1:1:pT
        ...
        iMarginalDist(ttt,:)= sum(iCurrentDistribution,2)';
        iEndoK(ttt)         = vGridA2' * iMarginalDist(ttt,:)';
...
Results.mMarginalWealth     = iMarginalDist;
